...
function openConnect(){
  sheet.classList.add("show");back.classList.add("show");
  accountId.value=state.canonicalId||state.accountId||"";
}
function closeConnect(){sheet.classList.remove("show");back.classList.remove("show")}
window.openConnect=openConnect;connectBtn.onclick=openConnect;closeSheet.onclick=closeConnect;back.onclick=closeConnect;
function valid(v){return /^[a-fA-F0-9]{24}$/.test(String(v||"").trim())}

function unwrapProfile(raw){
  let p=raw;
  if(typeof p==="string") p=JSON.parse(p);
  // Accept the exact Warframe response shape plus already-unwrapped profile objects.
  if(p?.Results?.[0]) return p;
  if(p?.results?.[0]) return p;
  if(p?.DisplayName || p?.displayName || p?.LoadOutInventory || p?.loadOutInventory) return p;
  if(p?.profile) return unwrapProfile(p.profile);
  throw new Error("This does not look like readable Warframe profile JSON.");
}
function storeImportedProfile(raw){
  const p=unwrapProfile(raw);
  state.profile=p;
  state.accountId=accountId.value.trim()||state.accountId;
  state.canonicalId=state.accountId||state.canonicalId;
...
function importText(text){
  ...
  if(!t) throw new Error("Nothing was copied.");
  if(/^https?:\/\//i.test(t)){
    throw new Error("iPhone copied the page address, not the JSON. On the JSON page press and hold the text → Select All → Copy, then return here.");
  }
  storeImportedProfile(JSON.parse(t));
}
...
if(pcProfileBtn) pcProfileBtn.onclick=()=>{
  ...
  if(!valid(id)){syncStatus.textContent="Enter the 24-character account ID first.";return}
  state.accountId=id;state.canonicalId=id;save();
  window.open(PC+encodeURIComponent(id),"_blank");
};

const clipboardImportBtn=document.getElementById("clipboardImportBtn");
if(clipboardImportBtn) clipboardImportBtn.onclick=async()=>{
  syncStatus.textContent="Reading copied text…";
  try{
    if(!navigator.clipboard?.readText) throw new Error("Clipboard reading is unavailable here. Open Paste manually instead.");
...
if(pasteImportBtn) pasteImportBtn.onclick=()=>{
  try{importText(document.getElementById("profilePaste").value)}
  catch(err){syncStatus.textContent=String(err?.message||err)}
};

const profileFile=document.getElementById("profileFile");
if(profileFile) profileFile.onchange=e=>{
  const f=e.target.files[0];if(!f)return;
  const r=new FileReader();
  r.onload=()=>{try{importText(r.result)}catch(err){syncStatus.textContent=String(err?.message||err)}};
  r.readAsText(f);
};

function exportBackup(){
  let b=new Blob([JSON.stringify(state,null,2)],{type:"application/json"}),a=document.createElement("a");
  a.href=URL.createObjectURL(b);a.download="warframe-tenno-deck-backup.json";a.click()
}
exportBtn.onclick=exportBackup;
resetBtn.onclick=()=>{if(confirm("Disconnect and reset local data?")){localStorage.removeItem(KEY);localStorage.removeItem("wfTennoDeckV2");location.reload()}};
