...
let state=Object.assign({accountId:"",canonicalId:"",profilePlatform:"PlayStation",profile:null,lastSync:null,source:null,items:{},collectionFilter:"Warframes"},previous,JSON.parse(localStorage.getItem(KEY)||"{}"));let catalogs={};const save=()=>localStorage.setItem(KEY,JSON.stringify(state));const esc=s=>String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
...
const sheet=document.getElementById("sheet"),back=document.getElementById("backdrop");function openConnect(){sheet.classList.add("show");back.classList.add("show");accountId.value=state.accountId||""}function closeConnect(){sheet.classList.remove("show");back.classList.remove("show")}window.openConnect=openConnect;connectBtn.onclick=openConnect;closeSheet.onclick=closeConnect;back.onclick=closeConnect;function valid(v){return /^[a-fA-F0-9]{24}$/.test(v.trim())}
async function fetchProfileEndpoint(base,id){
  const response=await fetch(base+encodeURIComponent(id),{cache:"no-store"});
  if(!response.ok)throw new Error("HTTP "+response.status);
  const text=await response.text();
  const redirect=text.match(/Retry with PC account:\s*([a-fA-F0-9]{24})(?:,([^\r\n<]+))?/i);
  if(redirect)return {redirectId:redirect[1],redirectName:(redirect[2]||"").trim(),raw:text};
  try{return {json:JSON.parse(text)}}catch{throw new Error("Unexpected profile response: "+text.slice(0,100))}
}
async function syncProfile(){
 let id=accountId.value.trim();
 if(!valid(id)){syncStatus.textContent="Account ID should be 24 hexadecimal characters.";return}
 state.accountId=id;save();
 syncStatus.textContent="Checking your PlayStation profile…";
 try{
   const ps=await fetchProfileEndpoint(PS,id);
   if(ps.json){
     state.profile=ps.json;state.canonicalId=id;state.profilePlatform="PlayStation";state.lastSync=Date.now();state.source="direct";save();render();
     syncStatus.textContent="PlayStation profile synced.";setTimeout(closeConnect,700);return;
   }
   if(ps.redirectId){
     state.canonicalId=ps.redirectId;state.profilePlatform="PC / Cross-Save";save();
     syncStatus.textContent=`Cross-Save detected${ps.redirectName?" for "+ps.redirectName:""}. Retrying automatically through PC…`;
     const pc=await fetchProfileEndpoint(PC,ps.redirectId);
     if(pc.json){
       state.profile=pc.json;state.lastSync=Date.now();state.source="direct";save();render();
       syncStatus.textContent="Cross-Save profile synced successfully.";setTimeout(closeConnect,800);return;
     }
     throw new Error("PC endpoint returned another redirect.");
   }
 }catch(err){
   syncStatus.textContent="Automatic sync was blocked or rate-limited. Use the PC/Cross-Save JSON button below first if your PlayStation page said “Retry with PC account.”";
 }
}
